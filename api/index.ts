...
// Load environment variables before anything reads process.env (e.g. Supabase keys).
// .env.local takes priority over .env; on hosts like Vercel neither file exists and
// the real process.env (set in the dashboard) is used instead. dotenv never overrides
// variables that are already defined, so this is safe in every environment.
